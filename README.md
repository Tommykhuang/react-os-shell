...
| `useUndo()` | `{ undo, redo, canUndo, canRedo, undoLabel, clear, baseline }` for the enclosing `UndoProvider` — for custom UI, to `clear()` the history after a save, or to `baseline()` it after a load. Call `baseline()` in the same effect that assigns a fetched record, or the load itself becomes the oldest step and the user's first ⌘Z hands back the empty form. `baseline()` discards the history only when a value actually lands behind it — reached on a refetch edge where the form's guard re-seeds nothing, it leaves the user's edits undoable; pass the record's id (`baseline(id)`) and a switch to another record drops the history even when its values happen to match. `clear()` always empties it: after a save, "earlier" is on the server. |
